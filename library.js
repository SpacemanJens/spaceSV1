class Flight {
  ...
  drawScore(offSetY) {
    ...
    let hitByOthers = this.hitByOtherFlights(flights);
...
class Canon {
  ...
  moveBullets() {
    ...
    for (let i = this.bullets.length - 1; i >= 0; i--) {
      ...
      if (!planet.isOnPlanet(bullet.xGlobal, bullet.yGlobal)) {
  ...
  syncFromShared(sharedCanon) {
    ...
    this.yGlobal = sharedCanon.yGlobal;
    this.diameter = sharedCanon.diameter;
    this.xSpawnGlobal = sharedCanon.xSpawnGlobal;
    this.ySpawnGlobal = sharedCanon.ySpawnGlobal;
    this.color = sharedCanon.color;
    this.bullets = sharedCanon.bullets
    this.hits = sharedCanon.hits
    this.planetIndex = sharedCanon.planetIndex
    this.angle = sharedCanon.angle
...
class Planet extends CelestialObject {
  ...
  onPlanet(xF, yF) {
    let posX = map(this.size / 2, 0, this.size, 0, 1500);
    let posY = map(this.size / 2, 0, this.size, 0, 1500);

    let distance = dist(xF, yF, posX, posY);
    //    console.log(int(distance));
    //    mainCanvas.fill(255)
    //    mainCanvas.text("Distance: " + distance, mouseX, mouseY + 40);
    let dMapped = map(this.size, 0, this.size, 0, 1500);
  ...
  drawFlight(flight) {
    ...
    //    fill(flight.color);
    fill('red');
    let posX = solarSystem.x + this.x + map(flight.xGlobal + flight.xLocal, 0, 1500, 0, this.size);
    let posY = solarSystem.y + this.y + map(flight.yGlobal + flight.yLocal, 0, 1500, 0, this.size);
    //    circle(posX, posY, 8);
    circle(posX, posY, 28);
  }
}
...
class SolarSystem {
  constructor(x, y) {
    ...
    this.planets = [
      ...
      new Planet(190, 0.4, 1100, 0.04, 1500, [0, 142, 144]),
